...
function nestedValuesMatch(result, queryValue, item, query, q) {
    return _.every(_.keys(queryValue), function (key) {
        if (operations.isOperation(key)) {
            return operations.getOperation(key)(resultObject(result, item), query[q], {query: query, queryItem: q});
        } else if (operations.isOperation(query[q][key])) {
            return operations.getOperation(query[q][key])(resultObject(result, item), query[q][key], {query: query, queryItem: key});
